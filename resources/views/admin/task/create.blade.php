...
@section('title', __('entities/task.add'))

@section('content')
<div class="container px-6 py-8 mx-auto">
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">{{ __('entities/task.add') }}</h2>
        </div>
        ...
        <form action="{{ route('admin.tasks.store') }}" method="POST">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    >
                        <option value="pending" @selected(old('status') == 'pending')>{{ __('entities/task.status_pending') }}</option>
                        <option value="in_progress" @selected(old('status') == 'in_progress')>{{ __('entities/task.status_in_progress') }}</option>
                        <option value="completed" @selected(old('status') == 'completed')>{{ __('entities/task.status_completed') }}</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
