...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <link rel="shortcut icon" href="/images/logo_color.svg" type="image/png">
        ...
        <div>
            <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
                ...
                <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="pt-10 fixed z-30 inset-y-0 left-0 max-w-48 transition duration-300 transform bg-gray-700 border-r-2 border-gray-300 overflow-y-auto lg:translate-x-0 lg:static lg:inset-0">
                    ...
                    <img src="/images/logo_color.svg" class="m-auto w-auto" alt="logo">

                    <nav class="mt-8 pt-8">
                        <a href="{{ route('admin.tasks.index') }}"
                            class="flex items-center mt-1 py-3 px-6 hover:text-yellow-500 no-underline
                            {{ Route::is('admin.tasks.*') ? ' bg-gray-900 text-gray-200' : ' text-white ' }}">
                                <i class="fa-solid fa-list-check"></i>
                                <span class="mx-3">{{ __('nav.tasks') }}</span>
                        </a>
                    </nav>
                </div>
